...
class CustomHangoutsMeet(models.Model):
    ...
    def create_attendees(self):
        ...
        for meeting in self:
            ...
            if partners_to_remove:
                attendees_to_remove = self.env["calendar.attendee"].search([('partner_id', 'in', partners_to_remove.ids),
                                                                            ('event_id', '=', meeting.id)])
    ...
    def send_mail_notification_mail(self):
        ...
        template_id = self.env['ir.model.data']._xmlid_to_res_id(
            'pragtech_odoo_hangout_meeting_integration.calendar_template_meeting_invitation_of_meeting_creation_call')
        login_user_id = self.env['res.users'].sudo().search([('id', '=', self._context.get('uid'))], limit=1)

        for attendee in self.attendee_ids:
            if attendee.partner_id != login_user_id.partner_id:
                email_template_obj = attendee.env['mail.template'].browse(template_id)

                if template_id:
                    values = email_template_obj.generate_email(attendee.id, ['subject', 'body_html', 'email_from', 'email_to',
                    ...
                    values['email_to'] = attendee.email
